startReactor = {                                 /* estruturas de controle */

    computerCombination: [],                     /* sequencia numérica que o computador gera ao dar play */
    playerCombination: [],                       /* sequencia numérica que o jgador faz */
    computerCombinationPosition: 1,              /* posição que o jogo está num determinado momento */
    combinationMaxPosition: 5,                   /* limite das posições, para saber se o jogo ja acabou */
    memoryMaxCombination: 9,                     /* máx de combinaçoes de memória, de possiibilidades */
    ...
    interface: {                         /* botões, painel, leds, através de atributos */
        ...
        async start() {               /* audio para recomeçar o jogo */
            return startReactor.audio.start.play()
        },

        playItem(index, combinationPosition, location = 'computer') {          /* o que faz o jogo funcionar */  
            ...
            startReactor.interface.turnLedOn(combinationPosition, leds)    /* qual é a vez que se esta jogando */
        ...
        /* quando erra ou quando termina o jogo */
        /* efeito piscando */
        ...
        endGame(type = "fail") {
            ...
            const audio = (type == "complete") ? startReactor.audio.complete : startReactor.audio.fail /* audio quando erra ou termina */
        ...
        enableButtons() {
            ...
            for (var i = 0; i < playerMemory.children.length; i++) {
                if (playerMemory.children[i].tagName == "DIV")
                    playerMemory.children[i].classList.add("playerMemoryActive");          /* teclado abilitado*/
            }
        ...
        disableButtons() {
            ...
            for (var i = 0; i < playerMemory.children.length; i++) {
                if (playerMemory.children[i].tagName == "DIV")
                playerMemory.children[i].classList.remove("playerMemoryActive");          /* teclado desabilitado */
            }
    ...
    async load() {                                     /* para carregar o jogo */
    ...
    start() {                                    /* para iniciar o jogo */
        ...
        startReactor.computerCombinationPosition = 1                  /* resetar as variaveis */
        startReactor.playerCombination = []
        startReactor.interface.start().then(() => {
            setTimeout(() => {
                startReactor.playCombination()                      /* tempo de espera para combinações */
            }, 500)
        })

    },                             

    createCombination() {                        /* gerando combinações aleatórias em 0 e 8 */
        let newCombination = []
